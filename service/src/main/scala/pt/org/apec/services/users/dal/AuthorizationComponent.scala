...
trait AuthorizationComponent[DT <: JdbcDriver] extends TablesSchema {
  ...
  class RolePermissions(tag: Tag) extends Table[(RoleId, PermissionId)](tag, "role_permissions") {
    ...
    def role = foreignKey("role_permission_fk", roleId, roles)(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
    def permission = foreignKey("permission_role_fk", permissionId, permissions)(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
  ...
  class UserRoles(tag: Tag) extends Table[(UserId, RoleId)](tag, "user_roles") {
    ...
    def roleId = column[RoleId]("role_id")
    def user = foreignKey("user_role_fk", userId, userEntities)(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
    def role = foreignKey("role_user_fk", roleId, roles)(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
  ...
  class UserPermissions(tag: Tag) extends Table[(UserId, PermissionId)](tag, "user_permissions") {
    ...
    def user = foreignKey("user_permissions_fk", userId, userEntities)(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
    def permission = foreignKey("permission_user_fk", permissionId, permissions)(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
